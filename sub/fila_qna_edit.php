...
<?php } ?>
		...
		<div id="container">
			...
			<section id="board">
				<h3>Q &amp; A 수정</h3>
				<form action="fila_qna_edit_control.php?no=<?=$row['no']?>" method="post">
					<fieldset>
						<legend>문의게시판 수정하기</legend>
						<div>
							<p>
								<label for="title">제목</label><input id="title" name="title" type="text" required value="<?=$row['title']?>"/>
							</p>
							<p class="username">
								<label for="username">이름</label><input id="username" name="username" type="text" required value="<?=$row['username']?>"/>
							</p>
							<p class="userpw">
								<label for="userpw">비밀번호</label><input id="userpw" name="userpw" type="password" required readonly value="<?=$row['userpw']?>"/>
							</p>
						</div>
						<p class="content">
