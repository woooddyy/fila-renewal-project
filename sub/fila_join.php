...
<html lang="ko">
 <head>
  ...
  <style>
	...
	/*########## container ##########*/
		#container{width:100%;}
		section{width:1200px; margin:30px auto 0;}
		#joinTitle .titleWrap{width:100%; padding-top:30px;}
		#joinTitle h3{font-size:23px; padding-bottom:7px;}
		#joinTitle .hr0{margin:20px 0; background-color:#333;}
		#joinTitle .subVisual, #joinTitle p, #joinTitle img{width:100%;}

		#joinContent form{width:100%;}
		#joinContent fieldset{border:none; margin-bottom:30px;}
		#joinContent legend{position:absolute; left:-9999px; width:1px; height:1px; line-height:0; font-size:1px; overflow:hidden;}
		#joinContent h3{font-size:16px; line-height:40px; border-bottom:1px solid #888;}
		#joinContent h3 em{font-size:13px; float:right; font-style:normal;}
		#joinContent span{color:#f00; vertical-align:middle;}
		#joinContent p{width:100%; padding:5px 0; font-size:14px; line-height:30px; height:30px; border-bottom:1px solid #ddd;}

		#basicContent label{display:inline-block; width:120px; text-indent:5px;}
		#basicContent label strong{font-weight:400;}
		#basicContent input{width:130px; height:28px; border:1px solid #ddd; text-indent:5px;}
		#basicContent input#addr{width:270px;}
		#basicContent em{font-style:normal; font-size:12px;}

		#additional dl{width:100%; margin-bottom:20px;}
		#additional dt{padding:10px 0 5px 5px;}
		#additional dd{float:left; margin-left:20px; line-height:22px;}
		#additional dd input{margin-right:3px; margin-top:5px;}
		#additional dd label em{font-style:normal; font-size:14px;}
		#additional dl:after{content:""; display:block; clear:both;}

		#btnArea{width:100%;}
		#btnArea .btn{width:400px; height:42px; margin:0 auto;}
		#btnArea p{border:none; width:200px; height:40px; line-height:40px; float:left; padding:0;}
		#btnArea p input{width:100%; height:100%; border:none; cursor:pointer; background-color:#fff;}
		#btnArea .reset input{border:1px solid #0B1B48; box-sizing:border-box;}
		#btnArea .submit input{background-color:#0B1B48; color:#fff;}
		#btnArea .reset:hover input{background-color:#ccc; color:#fff;}
		#btnArea .submit:hover input{background-color:#CA363B;}
	...
	/*################################ 반응형 맞추기 #######################################*/
		...
		@media all and (min-width:320px) and (max-width:479px){
			...
			section{width:300px;}
			#joinContent h3{font-size:14px; line-height:30px; margin-bottom:0;}
			#joinTitle p{font-size:12px;}
			#basicContent p{height:60px;}
			#basicContent label{display:block;}
			#additional dl{margin-bottom:10px;}

			#btnArea .btn{width:300px;}
			#btnArea p{width:150px;}
...
<?php } ?>
		...
		<div id="container">
			<h2>섹션 영역</h2>
			<section id="joinTitle">
				<h2>회원가입 타이틀 영역</h2>
				<div class="titleWrap">
					<h3>회원가입</h3>
					<p>FILA의 회원이 되어 다양한 혜택을 누려보세요</p>
				</div>
				<hr class="hr0"/>
				<div class="subVisual">
					<p><img src="../img/join_event.jpg" alt="회원가입이벤트"/></p>
				</div>
			</section>
			<section id="joinContent">
				<h2>회원가입 내용 영역</h2>
				<form action="fila_join_control.php" method="post">
					<fieldset id="basicContent">
						<legend>기본 정보 영역</legend>
						<h3>기본 정보 입력<em><span>*</span>은 필수 입력 사항입니다.</em></h3>
						<p>
							<label for="username"><strong>이름</strong> <span>*</span></label>
							<input id="username" name="username" type="text" required autocomplete="off" maxlength="10"/>
						</p>
						<p>
							<label for="userid"><strong>아이디</strong> <span>*</span></label>
							<input id="userid" name="userid" type="text" required autocomplete="off"	pattern="(?=.*\d)(?=.*[a-zA-Z]).{4,16}"/>
							<em>(영문 대소문자 / 숫자 4~16자)</em>
						</p>
						<p>
							<label for="userpw"><strong>비밀번호</strong> <span>*</span></label>
							<input id="userpw" name="userpw" type="password" required autocomplete="off"
							pattern="(?=.*\d)(?=.*[a-zA-Z]).{4,16}"/>
							<em>(영문 대소문자 / 숫자 4~16자)</em>
						</p>
						<p>
							<label for="userpwchk"><strong>비밀번호 확인</strong> <span>*</span></label>
							<input id="userpwchk" name="userpwchk" type="password" required autocomplete="off"
							pattern="(?=.*\d)(?=.*[a-zA-Z]).{4,16}"/>
						</p>
						<p>
							<label for="userphone"><strong>전화번호</strong> <span>*</span></label>
							<input id="userphone" name="userphone" type="tel" required autocomplete="off" placeholder="010-1234-1234" title="-를 넣고 기입" pattern="\d{3}-\d{3,4}-\d{4}"/>
						</p>
						<p>
							<label for="useremail"><strong>이메일</strong> <span>*</span></label>
							<input id="useremail" name="useremail" type="email" required autocomplete="off" placeholder="000@abc.co.kr" title="@를 넣고 기입"/>
						</p>
						<p>
							<label for="post"><strong>우편번호</strong> <span>*</span></label>
							<input id="post" name="post" type="text" required autocomplete="off" placeholder="00000" title="번호만입력해주세요"/>
						</p>
						<p>
							<label for="addr"><strong>상세주소</strong> <span>*</span></label>
							<input id="addr" name="addr" placeholder="00시 00구 00로 00, 00아파트 00동 00호" type="text" maxlength="50" required autocomplete="off" title="50자 제한입니다"/>
						</p>
					</fieldset>
					<fieldset id="additional">
						<legend>추가 정보 영역</legend>
						<h3>추가 정보 입력</h3>
						<dl>
							<dt><strong>성별</strong></dt>
							<dd>
								<input id="men" name="sex" type="radio"/><label for="men"><em>남성</em></label>
							</dd>
							<dd>
								<input id="women" name="sex" type="radio"/><label for="women"><em>여성</em></label>
							</dd>
						</dl>
						<dl>
							<dt><strong>관심상품</strong></dt>
							<dd>
								<input id="clothes" name="clothes" type="checkbox"/><label for="clothes"><em>의류</em></label>
							</dd>
							<dd>
								<input id="shoes" name="shoes" type="checkbox"/><label for="shoes"><em>신발</em></label>
							</dd>
							<dd>
								<input id="bag" name="bag" type="checkbox"/><label for="bag"><em>가방</em></label>
							</dd>
							<dd>
								<input id="acc" name="acc" type="checkbox"/><label for="acc"><em>악세서리</em></label>
							</dd>
						</dl>
					</fieldset>
					<fieldset id="btnArea">
						<legend>버튼영역</legend>
						<div class="btn">
							<p class="reset"><input type="reset" value="다시쓰기" title="다시쓰기"/></p>
							<p class="submit"><input type="submit" value="등록" title="등록"/></p>
						</div>
					</fieldset>
				</form>
			</section>
