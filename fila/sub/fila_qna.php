<?php
	...
	$sql="SELECT * FROM qnaboard ORDER BY no DESC";
	$result=mysqli_query($conn,$sql);

	$sqlCount="SELECT COUNT(*) FROM qnaboard";
	$allCount=mysqli_query($conn,$sqlCount);
	$rowCount=mysqli_fetch_array($allCount);
?>
<!DOCTYPE html>
<html lang="ko">
 <head>
  ...
  <style>
	...
	/*########## container ##########*/
		#container{width:100%;}
		#qnaWrap{width:1000px; margin:100px auto;}
		#qnaWrap h3{text-align:center; font-size:25px; color:#333; padding-bottom:20px; border-bottom:1px solid #000;}
		#qnaWrap table{width:100%; margin-top:30px;}
		#qnaboard caption{display:none;}
		#qnaboard thead{background-color:#121725; color:#fff;}
		#qnaboard th{line-height:32px; font-size:14px;}
		#qnaboard td{border-bottom:1px solid #ccc; text-align:center; font-size:13px; line-height:30px; padding:10px 0;}
		#qnaboard td a:hover, #qnaboard td a:focus{text-decoration:underline; color:#CC393C;}
		#qnaboard .clickArea td{text-align:right; border:none;}
		#qnaboard .clickArea a{padding:5px 10px; border:1px solid #333;}

		#qnaboard .clickArea a:hover, #qnaboard .clickArea a:focus{background-color:#121725; color:#fff;}
	...
	/*################################ 반응형 맞추기 #######################################*/
		...
		@media all and (min-width:768px) and (max-width:1024px){
			...
			#qnaWrap{width:700px; margin:50px auto;}
		...
		@media all and (min-width:480px) and (max-width:767px){
			...
			#qnaWrap{width:80%; margin:20px auto;}
...
<?php } ?>
		...
		<div id="container">
			...
			<section id="qnaWrap">
				<h3>1:1 문의사항</h3>
				<table id="qnaboard" title="문의사항 게시판">
					<caption>1:1 문의사항</caption>
					<colgroup>
						<col width="13%">
						<col width="32%">
						<col width="18%">
...
?>
						<tr>
							<td>
								<a href="./fila_qna_read.php?no=<?=$row['no']?>" title="자세히 보기"><?=$cnt+1?></a>
							</td>
							<td>
								<a href="./fila_qna_read.php?no=<?=$row['no']?>" title="자세히 보기"><?=$row['title']?></a>
							</td>
							<td><?=$row['username']?></td>
							<td><?=$row['mdate']?></td>
							<td><?=$row['view']?></td>
...
<?php $cnt++;} ?>
						<tr class="clickArea">
							<td colspan="5">
								<a href="./fila_qna_write.php" title="글쓰기">글쓰기</a>
							</td>
						</tr>
					</tbody>
				</table>
